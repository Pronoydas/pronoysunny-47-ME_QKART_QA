...
public class QKART_Tests {
    ...
    XSSFRow row = null;
    XSSFCell cell = null; 
    final String filePath ="/home/crio-user/workspace/pronoysunny-47-ME_QKART_QA/app/src/test/resources/TestResult.xlsx";
    ...
    @AfterMethod
    public void excelData(Method m,ITestContext testContext ,ITestResult testresult){
        String testMethodName=m.getName();
        String testStartDate=String.valueOf(testContext.getStartDate());
        String testEndDate = String.valueOf(testContext.getEndDate());
        String status= null;
      ...
      List<String> str = new ArrayList<String>();
       ...
       for (int i=0 ; i<=4;i++){
        ...
        switch(i){
            ...
            break;
            case 2:
            cell.setCellValue(testStartDate);
            break;
            case 3:
            cell.setCellValue(testEndDate);
            break;
            case 4:
   ...
   dataProvider = "data-provider" , dataProviderClass = DP.class)
    public void TestCase01(String TC1_Username, String TC1_Password)
    ...
    @Test(description = "Verify re-registering an already registered user fails", priority = 2, groups = {
            "Sanity_test" }, dataProvider = "data-provider" , dataProviderClass = DP.class ,enabled = false)
    ...
    ,dataProvider = "data-provider" , dataProviderClass = DP.class,enabled = false)
    ...
    public void TestCase03(String TC3_ProductNameToSearchFor) throws InterruptedException {
        ...
        assertFalse(isResultsEmpty, "Expected: no results, Actual: Results were available");
    }
    ...
    @Test(description = "Verify the existence of size chart for certain items and validate contents of size chart", priority = 4, groups = {
            "Regression_Test" } ,dataProvider = "data-provider" , dataProviderClass = DP.class,enabled = false)
    ...
    public void TestCase08(String TC8_ProductName, String TC8_Qty1) throws InterruptedException {
        Boolean status;
        int TC8_Qty = Integer.parseInt(TC8_Qty1);
        Register registration = new Register(driver);
        registration.navigateToRegisterPage();
        status = registration.registerUser("testUser", "abc@123", true);
        assertTrue(status, "Registration failed");
    ...
    @Test(description = "Verify that a product added to a cart is available when a new tab is added", priority = 10, dependsOnMethods = {
            "TestCase10" }, groups = { "Regression_Test" },enabled = false)
