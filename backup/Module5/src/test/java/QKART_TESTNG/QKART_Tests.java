...
public class QKART_Tests {
    ...
    public static void TestCase02(String userName, String password) throws InterruptedException {
        Boolean status;
        ...
        Assert.assertTrue(status, "Test Case 2: Verify user Registration : " +String.valueOf(status ? "PASS" : "FAIL"));
     ...
     */
    @Parameters({"vaildProductName","inVaildProductName"})
    @Test(description ="Verify the functionality of search text box"  , priority = 3, groups={"Sanity_test"})
    public static void TestCase03(@Optional("yonex") String vaildProductName , @Optional("Gesundheit") String inVaildProductName) throws InterruptedException {
        ...
        status = homePage.searchForProduct(vaildProductName);
        if (!status) {
        ...
        // Search for product
        status = homePage.searchForProduct(inVaildProductName);
        if (!status) {
     ...
     */
    @Parameters ("ShoeName")
    @Test(description = "Verify the existence of size chart for certain items and validate contents of size chart" ,priority =4  , groups={"TestCase04()"})
    public static void TestCase04(@Optional("Roadster")String ShoeName) throws InterruptedException {
        ...
        status = homePage.searchForProduct(ShoeName);
        List<WebElement> searchResults = homePage.getSearchResults();
        ...
        for (WebElement webElement : searchResults) {
            ...
            if (result.verifySizeChartExists()) {
                ...
                Assert.assertTrue(status, "Size DropDown is not Exist");
                ...
                if (result.openSizechart()) {
                    ...
                    Assert.assertTrue(result.validateSizeChartContents(expectedTableHeaders, expectedTableBody, driver), "Failure while validating contents of Size Chart Link "+"FAIL");
                    logStatus("Step Success", "Successfully validated contents of Size Chart Link", "PASS");
    ...
    public static void TestCase05(String productName1 ,String productName2 ,String address) throws InterruptedException {
        Boolean status;
        ...
        Assert.assertTrue(status, "User Perform Login Failed ,Test Case 5: Happy Flow Test Failed .");
     ...
     */
    @Parameters({"address","TC6_ProductNameToSearch1","TC6_ProductNameToSearch2"})
    @Test(description = "Verify that the contents of the cart can be edited" , priority =6  , groups={"Regression_Test"})
    public static void TestCase06(String TC6_ProductNameToSearch1 , String TC6_ProductNameToSearch2 ,String address) throws InterruptedException {
        Boolean status;
        ...
        registration.navigateToRegisterPage();
        registration.registerUser("Username", "abcabc", true);
        String LastRegisterUser = registration.lastGeneratedUsername;

        // TODO: Login using the newly registed user
        login.navigateToLoginPage();
        Thread.sleep(3000);
        login.PerformLogin(LastRegisterUser, "abcabc");
        ...
        Thread.sleep(1000);
       homePage.verifyCartContents(Arrays.asList("Xtend Smart Watch"));
       Thread.sleep(1000);
        homePage.clickCheckout();
        ...
        status = driver.getCurrentUrl().endsWith("/thanks");
        Assert.assertTrue(status,"Test Case 6: Verify that cart can be edited: "+ String.valueOf(status ? "PASS" : "FAIL"));
     ...
     */
    @Parameters({"TC7_ListOfProductsToAddToCart" ,"TC7_ListOfProductsToAddToCart1" , "userName","password"})
    ...
    public static void TestCase07(String TC7_ListOfProductsToAddToCart , String TC7_ListOfProductsToAddToCart1, String userName ,String password ) throws InterruptedException {
        Boolean status = false;
        ...
        Assert.assertTrue(status, "User Perform Login Failed , Test Case 7:  Verify that cart contents are persited after logout: ");
        lastGeneratedUserName = registration.lastGeneratedUsername;
        ...
        status = homePage.searchForProduct(TC7_ListOfProductsToAddToCart1);
        homePage.addProductToCart(TC7_ListOfProductsToAddToCart1);
    ...
    @Parameters({"userName","password","address","TC_08_product"})
    @Test(description = "Verify that insufficient balance error is thrown when the wallet balance is not enough" , priority = 8, groups={"TestCase08()"})
    public static void TestCase08(String userName , String password , String address , String TC_08_product) throws InterruptedException {
        Boolean status;
        ...
        Home homePage = new Home(driver);
        homePage.navigateToHome();
        status = homePage.searchForProduct(TC_08_product);
        homePage.addProductToCart(TC_08_product);

        homePage.changeProductQuantityinCart(TC_08_product, 10);
