...
public class SearchResult {
    ...
    public String getTitleofResult() {
        ...
        return titleOfSearchResult= parentElement.findElement(By.className("css-yg30e6")).getText();
    }
    ...
    public Boolean verifySizeChartExists() {
       // Boolean status = false;
        try {
             ...
             */
            String str=parentElement.findElement(By.tagName("button")).getText();
            return str.equalsIgnoreCase("Size chart") ? true :false ;
        } catch (Exception e) {
            return false;
        }
    }
    ...
    public Boolean validateSizeChartContents(List<String> expectedTableHeaders, List<List<String>> expectedTableBody,
        ...
        try {
            ...
            for(WebElement e: tableHeader){
               if (!e.getText().equalsIgnoreCase(expectedTableHeaders.get(index))){
                   status=false;
               }
              index++; 
            }
            Thread.sleep(3000);
           //int sizeTable=driver.findElements(By.xpath("")).size();
           List<WebElement> list=driver.findElements(By.tagName("tbody"));
           for (int i=0 ; i<expectedTableBody.size();i++){
               List<WebElement> nthRow=list.get(i).findElements(By.tagName("td"));
               for(int j =0 ; j<expectedTableBody.get(i).size(); j++){
                 String expected=expectedTableBody.get(i).get(j);
                 String actual = nthRow.get(j).getText();
    ...
    public Boolean verifyExistenceofSizeDropdown(WebDriver driver) {
        ...
        try {
           ...
           if(sizeChart.isDisplayed() && sizeChart.isEnabled()){
            status=true;
           }else{
            status=false;
           }
