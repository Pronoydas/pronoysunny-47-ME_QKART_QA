...
public class QkartSanity {
    ...
    public static void takeScreenshot(WebDriver driver, String screenshotType, String description)  {
        ...
        System.out.println("Error Getting From ScreenShort "+e.getMessage());
    }
    ...
    public static Boolean TestCase01(RemoteWebDriver driver) throws InterruptedException {
        ...
        if (!status) {
            takeScreenshot(driver, "Fail_UserRegistation", "TESTCASE01");
            ...
            takeScreenshot(driver, "UserRegistation", "TESTCASE01");
        ...
        takeScreenshot(driver, "UserLogin", "TESTCASE01");
    ...
    public static Boolean TestCase03(RemoteWebDriver driver) throws InterruptedException {
        ...
        status = homePage.searchForProduct("yonex");//YONEX
        if (!status) {
    ...
    public static Boolean TestCase07(RemoteWebDriver driver) throws InterruptedException {
        ...
        login.navigateToLoginPage();
        status = login.PerformLogin(LastRegisterUser, "abcabc");
        Thread.sleep(3000);
    ...
    public static Boolean TestCase09(RemoteWebDriver driver) throws InterruptedException {
        ...
        logStatus("TestCsae09", "Start TestCase09", "Done");
        try{
        Register r = new Register(driver);
        r.navigateToRegisterPage();
        status =r.registerUser("Sunny", "abc#345", true);
        if(!status){
            takeScreenshot(driver, "TestCase09", "ErrorTimeofRegistration");
            logStatus("TestCase09", "Error At the of Registration", "Fail");
        }
        String lastRegisterUser=r.lastGeneratedUsername;
        Login login = new Login(driver);
        login.navigateToLoginPage();
        status=login.PerformLogin(lastRegisterUser, "abc#345");
        if(!status){
            takeScreenshot(driver, "TestCase09", "ErrorTimeofLogin");
            logStatus("TestCase09", "Error At the of LogIN", "Fail");
        }
        Home home = new Home(driver);
        status =home.searchForProduct("YONEX Smash Badminton Racquet");
        if(!status){
            takeScreenshot(driver, "TestCase09", "ErrorTimeofProductSearch");
            logStatus("TestCase09", "Error At the of Product search", "Fail");
        }
       status=home.addProductToCart("");
       if(!status){
        takeScreenshot(driver, "TestCase09", "ErrorTimeofaddingcart");
        logStatus("TestCase09", "Error At the of Adding Cart", "Fail");
       }
       takeScreenshot(driver, "TestCase09", "AfterAddingtoCart");
       Thread.sleep(2000);
       ...
       login.navigateToLoginPage();
        ...
        WebDriverWait wdw = new WebDriverWait(driver,30);
        wdw.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//div[@class='MuiBox-root css-zgtx0t']")));
        takeScreenshot(driver, "TestCase09", "AfterAddingtoCartintab");
        driver.close();
        driver.switchTo().window(parentWindow);
        status= home.PerformLogout();
        logStatus("Testcase09", "Successfully Validate TestCase", "Pass");
        return status;
    }catch(Exception e){
       takeScreenshot(driver, "TestCase09", "Fail");
       System.out.println("Test Case fail due to "+ e.getMessage());
       return false;
    }
    }
    ...
    public static Boolean TestCase10(RemoteWebDriver driver) throws InterruptedException {
        ...
        status=home.clickOnLinks(privacyLink);
        if(!status){
            takeScreenshot(driver, "TestCase10", "UnableToClickOnLink");
            logStatus("TestCase10", "Issue occur at time of Click On Privacy policy Link", status ? "Pass":"Fail");
        }else{
            logStatus("TestCase10", "Successfully Click On Privacy policy Link", status ? "Pass":"Fail");
        }
        Thread.sleep(1000);
        status=home.verifyCurrentUrl(driver);
        if(!status){
            logStatus("TestCase10", "Current Url Changes ", status ? "Pass":"Fail");
        }else{
            logStatus("TestCase10", "Successfully Validate the URL", status ? "Pass":"Fail");
        }
        status=home.switchToTab(driver);
        if(!status){
            takeScreenshot(driver, "TestCase10", "SWITCHTABISSUE");
            logStatus("TestCase10", "Issue occur at time of Switch to Privacy policy Tab", status ? "Pass":"Fail");
        }else{
            logStatus("TestCase10", "Successfully Switch to Privacy policy Tab", status ? "Pass":"Fail");
        }
        WebElement privacyLink1=driver.findElement(By.xpath("//h2[normalize-space()='Privacy Policy']"));
        status=home.verifyTabContent(privacyLink1, "Privacy policy");
        if(!status){
            takeScreenshot(driver, "TestCase10", "SWITCHTABISSUE");
            logStatus("TestCase10", "Context not match", status ? "Pass":"Fail");
        }else{
            logStatus("TestCase10", "Successfully Validate Privacy policy Tab", status ? "Pass":"Fail");
        }
        takeScreenshot(driver, "TestCase10", "PrivacyPolicy");
        status= home.switchToParentWindow(driver);
        if(!status){
            takeScreenshot(driver, "TestCase10", "SWITCHTABISSUE");
            logStatus("TestCase10", "Error While Switch Back to Parent", status ? "Pass":"Fail");
        }else{
            logStatus("TestCase10", "Successfully SwitchBack to ParentTab", status ? "Pass":"Fail");
        }
        ////Terms of Service
        WebElement termsLink=driver.findElement(By.xpath("//a[normalize-space()='Terms of Service']"));
        status=home.clickOnLinks(termsLink);
        if(!status){
            takeScreenshot(driver, "TestCase10", "UnableToClickOnLink");
            logStatus("TestCase10", "Issue occur at time of Click On Terms of Service Link", status ? "Pass":"Fail");
        }else{
            logStatus("TestCase10", "Successfully Click On Terms of Service Link", status ? "Pass":"Fail");
        }
        Thread.sleep(1000);
        status=home.verifyCurrentUrl(driver);
        if(!status){
        ...
        if(!status){
            takeScreenshot(driver, "TestCase10", "SWITCHTABISSUE");
            logStatus("TestCase10", "Issue occur at time of Switch to Terms of Service Tab", status ? "Pass":"Fail");
        }else{
            logStatus("TestCase10", "Successfully Switch to Terms of Service Tab", status ? "Pass":"Fail");
        }
        WebElement termaText=driver.findElement(By.xpath("//h2[normalize-space()='Terms of Service']"));
        status=home.verifyTabContent(termaText, "Terms of Service");
        if(!status){
            takeScreenshot(driver, "TestCase10", "SWITCHTABISSUE");
            logStatus("TestCase10", "Context not match", status ? "Pass":"Fail");
        }else{
            logStatus("TestCase10", "Successfully Validate Terms of Service Tab", status ? "Pass":"Fail");
        }
        takeScreenshot(driver, "TestCase10", "TermsOfService");
        status= home.switchToParentWindow(driver);
        if(!status){
            takeScreenshot(driver, "TestCase10", "SWITCHTABISSUE");
            logStatus("TestCase10", "Error While Switch Back to Parent", status ? "Pass":"Fail");
    ...
    public static Boolean TestCase11(RemoteWebDriver driver) throws InterruptedException {
        ...
        Home home = new Home(driver);
        home.navigateToHome();
        WebElement contactUsBtw =driver.findElement(By.xpath("//p[normalize-space()='Contact us']"));
        status=home.clickOnLinks(contactUsBtw);
        if(!status){
            takeScreenshot(driver, "TestCase11", "UnableToClickOnLink");
            logStatus("TestCase11", "Issue occur at time of Click On Contact us", status ? "Pass":"Fail");
        }else{
            logStatus("TestCase11", "Successfully Click On Contact us Link", status ? "Pass":"Fail");
        }
        status=home.contractUs(driver);
        if(!status){
           // takeScreenshot(driver, "TestCase11", "UnableToPerfromOperation");
            logStatus("TestCase11", "Issue occur at time of sending values On Contact us", status ? "Pass":"Fail");
        }else{
            logStatus("TestCase11", "Successfully Validate On Contact us Link", status ? "Pass":"Fail");
        }
        return status;
    }
    ...
    public static Boolean TestCase12(RemoteWebDriver driver) throws InterruptedException {
        ...
        WebDriverWait wdw = new WebDriverWait(driver, 30);
        wdw.until(ExpectedConditions.urlContains("/thanks"));
        logStatus("TestCase12", "Order Place Successfully", "Pass");

        status=checkoutPage.verifyCountOfAdvertisements(driver);
        if (!status){
            logStatus("TestCase12", "Advertisement's Count is not Matching", "Fail");
        }
        else {
            logStatus("TestCase12", "Advertisement's Count is  Matched", "Pass");
        }
        status=checkoutPage.verifyContentOfAdvertisements(driver);
        if (!status){
            logStatus("TestCase12", "Advertisement's Content is not Matching", "Fail");
        }
        else {
            logStatus("TestCase12", "Advertisement's Content is  Matched", "Pass");
        }
        return status;
    }
